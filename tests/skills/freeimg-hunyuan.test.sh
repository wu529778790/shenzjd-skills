...
echo ""
echo "📋 Test 4: 离线参数校验"
out="$(env -u TCB_ENV_ID -u TCB_SECRET_ID -u TCB_SECRET_KEY -u FREEIMG_HUNYUAN_CONFIG HOME="$TEST_DIR/no-home" node "$SKILL_DIR/scripts/generate.mjs" --prompt x --out /tmp/freeimg-hunyuan-test.jpg 2>&1 || true)"
...
out="$(TCB_ENV_ID=e TCB_SECRET_ID=s TCB_SECRET_KEY=k node "$SKILL_DIR/scripts/generate.mjs" --out /tmp/freeimg-hunyuan-test.jpg 2>&1 || true)"
